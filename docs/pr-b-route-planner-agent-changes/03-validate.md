# server/src/services/ai/route-planner-agent/validate.js
...
export function normalizeActionPayload(actionType, payload, field = 'action.payload', code = ERROR_CODES.INVALID_ACTION_PAYLOAD) {
  ...
  const targetRegionKey = normalizeNullableString(payload.target_region_key);
  return ok(targetRegionKey ? { target_region_key: targetRegionKey } : {});
}
